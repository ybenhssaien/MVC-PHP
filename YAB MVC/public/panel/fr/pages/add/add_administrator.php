<div class="widgetbox box-inverse modal" id="modalparent">
	<form action="javascript:;" id="addform">
		<div class="row-fluid">
			<div class="span12">
				<div class="widgetbox">
					<h4 class="widgettitle">Création d'un administrateur</h4>
					...
					<?php if(isset($request["noerror"]) && $request["noerror"]){ ?>
						<script>jQuery("#close").click();successDialog("Ajout est efféctué avec succés");setTimeout(function(){reload();},2000);</script>
					<?php } ?>
...
<script>
	jQuery(".add").click(function(){submitAdd();});
	jQuery('#modalparent').css({"left":"40%","top":"15%"});
	jQuery(".oblg").css("color","red");
	jQuery(".oblg").attr("title","champ obligatoire");
	function submitAdd(){
		jQuery('#modalparent').parent().load('<?php echo $adminO::getAdminURL().CURRENT_PAGE_NAME ?>/ajax/add',jQuery("#addform").serialize());
	}
</script>